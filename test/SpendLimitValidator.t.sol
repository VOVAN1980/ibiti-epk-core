...
contract SpendLimitValidatorTest is Test {
    ...
    function _policyWithValidator() internal returns (uint256 pid) {
        ...
        // РІРєР»СЋС‡Р°РµРј Р»РёРјРёС‚С‹ РґР»СЏ native token (address(0))
        vm.prank(owner);
    ...
    function testSpendLimit_perTx_native() public {
        ...
        // Р»РёРјРёС‚ perTx = 1000 (wei/units), РѕРєРЅРѕ Р±РѕР»СЊС€РѕРµ С‡С‚РѕР±С‹ РЅРµ РјРµС€Р°Р»Рѕ
        vm.prank(owner);
    ...
    function testSpendLimit_rollingWindow_native() public {
        uint256 pid = _policyWithValidator();
        // perTx РІС‹СЃРѕРєРёР№, РѕРєРЅРѕ = 2000
        vm.prank(owner);
        ...
        // 3) +1 => СЂРµРІРµСЂС‚ (PerWindowExceeded)
...
contract SpendLimitValidatorExtraTest is SpendLimitValidatorTest {
    ...
    function testValidate_unauthorizedCaller_reverts() public {
        // РџСЂСЏРјРѕР№ РІС‹Р·РѕРІ validate РЅРµ РѕС‚ kernel
        vm.prank(agent);
    ...
    function testValidate_dataTooShort_returns_noRevert() public {
        uint256 pid = _policyWithValidator();
        // value=0 Рё data.length<4 => РїСЂРѕСЃС‚Рѕ return
    ...
    function testValidate_perWindowResetAfterWindow_passes() public {
        ...
        // РјРѕС‚Р°РµРј РІСЂРµРјСЏ Р·Р° РѕРєРЅРѕ
        vm.warp(block.timestamp + 1 days + 1);
        // С‚РµРїРµСЂСЊ СЃРЅРѕРІР° РґРѕР»Р¶РЅРѕ РїСЂРѕС…РѕРґРёС‚СЊ
        uint256 d3 = block.timestamp + 1 hours;
        uint256 n3 = kernel.nonces(pid);
        bytes memory s3 = _sign(pid, address(target), 1, data, d3, n3);
        vm.prank(agent);
    ...
    function testApprove_nonZero_reverts() public {
        uint256 pid = _policyWithValidator();
        // Р Р°Р·СЂРµС€Р°РµРј call approve РЅР° target, С‡С‚РѕР±С‹ РґРѕР№С‚Рё РґРѕ РІР°Р»РёРґР°С‚РѕСЂР°
        vm.prank(owner);
        kernel.setCall(pid, address(target), bytes4(0x095ea7b3), true);
        // include native limits enabled, РЅРѕ value=0
        vm.prank(owner);
        ...
        vm.prank(agent);
        vm.expectRevert(abi.encodeWithSelector(SpendLimitValidator.NonZeroApprove.selector, address(target), uint256(123)));
        kernel.execute(pid, address(target), 0, data, deadline, sig);
    }
    function testApprove_zero_passes() public {
    // создаём policy как обычно
    uint256 pid = _policyWithValidator();

    // ВАЖНО: этот тест проверяет только логику валидатора,
    // поэтому вызываем validate напрямую от authorizedCaller (kernel)
    ...
    vm.prank(address(kernel)); // authorizedCaller для validator
    ...
    function testTransfer_tokenLimitDisabled_reverts() public {
        ...
        // native enabled (С‡С‚РѕР±С‹ РЅРµ СѓРїР°СЃС‚СЊ СЂР°РЅСЊС€Рµ), token(target) disabled
        vm.prank(owner);
        validator.setLimits(pid, address(0), uint128(1e18), uint128(1e18), uint32(1 days), true);
        vm.prank(owner);
    ...
    function testTransfer_badCalldata_reverts() public {
        ...
        // selector transfer + РЅРµРґРѕСЃС‚Р°С‚РѕС‡РЅРѕ calldata (РЅРµС‚ amount arg РїРѕР»РЅРѕСЃС‚СЊСЋ)
